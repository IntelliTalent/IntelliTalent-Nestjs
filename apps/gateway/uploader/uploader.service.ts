...
export class UploaderService {
  ...
  private async _getBlobClient(fileName: string): Promise<BlockBlobClient> {
    const containerName = await this.configService.get(Constants.AZURE.AZURE_STORAGE_CONTAINER);
    const containerClient = this.blobClientService.getContainerClient(containerName);
