...
describe('Filtering service test', () => {
  ...
  // Tests for passQuiz
  it('should pass quiz and move to interview stage', async () => {
    const userId = uuidv4();
    const jobId = uuidv4();
    const profileId = uuidv4();
    const grade = 85;
    ...
    const result = await filteringService.passQuiz(userId, jobId, profileId, grade);
    ...
    expect((result.stageData as InterviewData).grade).toBe(85);
  });

  it('should pass quiz and become a candidate', async () => {
    const userId = uuidv4();
    const jobId = uuidv4();
    const profileId = uuidv4();
    const grade = 90;
    ...
    const job = {
      ...
      userId: userId,
      stages: {
        interview: null,  // Job does not have an interview stage
      },
    } as StructuredJob;
    ...
    expect(result).toBeDefined();
    expect(result).toHaveProperty('stage');
    expect(result.stage).toBe(StageType.candidate);
    expect(result).toHaveProperty('stageData');
    expect(result.stageData).toHaveProperty('grade');
    expect((result.stageData as InterviewData).grade).toBe(90);
  });

  it('pass quiz with user not owner of the job, should throw exception', async () => {
    const userId = uuidv4();
    const jobId = uuidv4();
    const profileId = uuidv4();
    const grade = 75;
    ...
    await expect(filteringService.passQuiz(userId, jobId, profileId, grade)).rejects.toThrow(FILTERATION_CONSTANTS.USER_NOT_JOB_OWNER);
  });

  it('pass quiz with user did not apply, should throw exception', async () => {
    const userId = uuidv4();
    const jobId = uuidv4();
    const profileId = uuidv4();
    const grade = 80;
    ...
    await expect(filteringService.passQuiz(userId, jobId, profileId, grade)).rejects.toThrow(FILTERATION_CONSTANTS.USER_DID_NOT_APPLY);
  });

  it('pass quiz with user not in quiz stage, should throw exception', async () => {
    const userId = uuidv4();
    const jobId = uuidv4();
    const profileId = uuidv4();
    const grade = 80;
    ...
    filteration.userId = userId;
    filteration.email = 'test@gmail.com';
    filteration.currentStage = StageType.applied;  // Not in quiz stage
    ...
    await expect(filteringService.passQuiz(userId, jobId, profileId, grade)).rejects.toThrow(FILTERATION_CONSTANTS.USER_IS_NOT_IN_QUIZ_STAGE);
  });

  // Tests for failQuiz
  it('should fail quiz and move to failed stage', async () => {
    const userId = uuidv4();
    const jobId = uuidv4();
    const profileId = uuidv4();
    const grade = 50;
    ...
    const result = await filteringService.failQuiz(userId, jobId, profileId, grade);

    expect(result).toBeDefined();
    expect(result).toHaveProperty('stage');
    expect(result.stage).toBe(StageType.failed);
    expect(result).toHaveProperty('stageData');
    expect(result.stageData).toHaveProperty('grade');
    expect((result.stageData as InterviewData).grade).toBe(50);
    expect(result.message).toBe(FILTERATION_CONSTANTS.USER_FAILED_QUIZ);
  });

  it('fail quiz with user not owner of the job, should throw exception', async () => {
    const userId = uuidv4();
    const jobId = uuidv4();
    const profileId = uuidv4();
    const grade = 40;
    ...
    await expect(filteringService.failQuiz(userId, jobId, profileId, grade)).rejects.toThrow(FILTERATION_CONSTANTS.USER_NOT_JOB_OWNER);
  });

  it('fail quiz with user did not apply, should throw exception', async () => {
    const userId = uuidv4();
    const jobId = uuidv4();
    const profileId = uuidv4();
    const grade = 30;
    ...
    await expect(filteringService.failQuiz(userId, jobId, profileId, grade)).rejects.toThrow(FILTERATION_CONSTANTS.USER_DID_NOT_APPLY);
  });

  it('fail quiz with user not in quiz stage, should throw exception', async () => {
    const userId = uuidv4();
    const jobId = uuidv4();
    const profileId = uuidv4();
    const grade = 20;
    ...
    await expect(filteringService.failQuiz(userId, jobId, profileId, grade)).rejects.toThrow(FILTERATION_CONSTANTS.USER_IS_NOT_IN_QUIZ_STAGE);
  });
